...
PerdoruesId
  ...
  while($info=mysqli_fetch_array($data))
      echo $info['PerdoruesId'].','; /* We use the concatenation operator '.' to add comma delimiters after each data value. */
  ?>];
  ...
  while($info1=mysqli_fetch_array($data1))
      echo $info1['PerdoruesId'].','; /* We use the concatenation operator '.' to add comma delimiters after each data value. */
  ?>];
...
while($info1=mysqli_fetch_array($tezena))
    echo $info1['DhomaId'].','; /* We use the concatenation operator '.' to add comma delimiters after each data value. */
?>];
