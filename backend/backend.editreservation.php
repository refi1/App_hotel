	...
	$data =  date('Y-m-d');
	    ...
	    if (empty($mberritja) || empty($largimi) || empty($tipiIdhomes) || empty($persona)) {
	        $_SESSION['error'] = 'Ju lutem plotësoni të gjitha hapësirat!';
					 header("Location: ../editreservation.php?RezervimId=".$RezervimId);
	        exit();
	    } else {
	      if(empty($qendrimi) || $persona <= 0 || $cmimi <=0) {
					$_SESSION['error'] = 'Ju lutem vendosni data dhe numrin e personave saktë!';
	        header("Location: ../editreservation.php?RezervimId=".$RezervimId);
	        exit();
	      }
        ...
        AND DhomaId=".$dhomaid;
            ...
            header("location:../editreservation.php?reservation=error");
            exit();
         }
			 }
		 }
